...
tidy_modelo <- tidy(mod_h1)
tidy_modelo <- tidy_modelo %>%
  mutate(term = recode(term,
                       `(Intercept)` = "Intercepto",
                       `bmi` = "Índice de Masa Corporal (BMI)",
                       `height` = "Altura (en mts.)",
                       `experience` = "Experiencia (en años)",
                       `age` = "Edad (en años)")) %>%
  select(term, estimate, std.error, statistic, p.value) %>%
  mutate(
    p.value = case_when(p.value < 2e-16 ~ "< 2e-16",
                        TRUE ~ formatC(p.value, digits = 4, format = "e")
    ),
...
kable(tidy_modelo, caption = "Resultados del modelo lineal")
```
...
tidy_modelo2 <- tidy(mod_h2_estandarizado$finalModel)
tidy_modelo2 <- tidy_modelo2 %>%
  mutate(term = recode(term,
                       `(Intercept)` = "Intercepto",
                       `bmi` = "Índice de Masa Corporal (BMI)",
                       `height` = "Altura (en mts.)",
...
kable(tidy_modelo2, caption = "Resultados del modelo lineal estandarizado")

kable(
  glance(mod_h2_estandarizado$finalModel)[, c("r.squared", "adj.r.squared")],
  caption = "R² del modelo lineal estandarizado"
)
```
...
tidy_modelo3 <- tidy(mod_m2_estandarizado$finalModel)
tidy_modelo3 <- tidy_modelo3 %>%
  mutate(term = recode(term,
                       `(Intercept)` = "Intercepto",
                       `bmi` = "Índice de Masa Corporal (BMI)",
                       `height` = "Altura (en mts.)",
...
kable(tidy_modelo3, caption = "Resultados del modelo lineal estandarizado")

kable(
  glance(mod_m2_estandarizado$finalModel)[, c("r.squared", "adj.r.squared")],
  caption = "R² del modelo lineal estandarizado")
```
